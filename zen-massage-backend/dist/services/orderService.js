...
async function createCommande(data) {
    ...
    for (const item of data.items) {
        const produit = await prisma_1.prisma.produit.findUnique({ where: { id: item.produit_id } });
        if (!produit) {
            const err = new Error(`Produit introuvable : ${item.nom_produit}`);
            err.status = 404;
            throw err;
        }
        if (produit.stock < item.quantite) {
            const err = new Error(`Stock insuffisant pour "${produit.nom}" (disponible: ${produit.stock})`);
            err.status = 409;
            throw err;
        }
    }
    ...
    const commande = await prisma_1.prisma.$transaction(async (tx) => {
...
async function getCommandesUtilisateur(userId) {
...
async function getCommandeById(id, userId, role) {
...
async function cancelCommande(id, userId) {
    const commande = await prisma_1.prisma.commande.findUnique({
        where: { id },
        include: { lignes: true },
    });
    ...
    if (commande.statut !== 'PENDING' && commande.statut !== 'CONFIRMED') {
        const err = new Error('Cette commande ne peut plus être annulée');
        err.status = 400;
        throw err;
    }
